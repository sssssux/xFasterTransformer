...
std::vector<int32_t> Model::generate() {
    ...
    if (searcher != nullptr) {
        ...
        if (isNewInput) {
            ...
            if (ctx->ppSize > 1 && ctx->ppRank == 0 ) {
                isNewInput = false;
                int *ids = inputIds.data();
                std::vector<int32_t> inputTokens(seqLen);
                for (int i = 0; i < batchSize; ++i) {
                    // st batch index i sequence to inputTokens
                    std::copy(ids + i * seqLen, ids + (i + 1) * seqLen, std::back_inserter(inputTokens));
                    auto *group = SequencePool::getInstance().newGroupMeta(inputTokens);
                    InputQueue::getInstance().push(group);
                }
            }
            return searcher->getNextToken(inputIds.data(), batchSize, seqLen);
        }else {
